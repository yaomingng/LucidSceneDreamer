...
def main():
    ...
    # batch_size = cfg.data.train.batch_size # this will be modified to be 1
    # total_step = max(cfg.trainer.dis_step, cfg.trainer.gen_step) #max(1, 1)
    ...
    net_G, _, opt_G, _, sch_G, _ = \
        get_model_optimizer_and_scheduler(cfg, seed=args.seed) # removed net_D, opt_D, and sch_D
    trainer = get_trainer(cfg, net_G, None,
                          opt_G, None,
                          sch_G, None,
                          train_data_loader, val_data_loader) # removed net_D, opt_D, and sch_D
    ...
    if is_master():
        ...
        # wandb.watch(trainer.net_D.module) #removed
    ...
    for epoch in range(current_epoch, cfg.max_epoch):
        ...
        for it, data in enumerate(train_data_loader):
            with profiler.profile(enabled=args.profile,
                ...
                trainer.gen_update(data) # changed to only do generator update
